...
$title_page = get_field('mba_title_page', $page_id);
$review_logo = get_field('mba_logo', $page_id);
$button = get_field('mba_buttons', $page_id);
$review_rating = get_field('mba_rating', $page_id);
$choose_bg = get_field('mba_image_color', $page_id);
$review_bg_image = !empty(get_field('mba_bg_image', $page_id)) ? 'style="background-image: url('. get_field('mba_bg_image', $page_id) . ')"' : '';
$review_bg_color = !empty(get_field('mba_bg_color', $page_id)) ? 'style="background-color: '. get_field('mba_bg_color', $page_id) . '"' : '';
$review_bg_logo = !empty(get_field('mba_bg_color_logo', $page_id)) ? 'style="background-color: '. get_field('mba_bg_color_logo', $page_id) . '"' : '';
$dark_bg = get_field('mba_dark_bg') ? 'dark-bg' : '';
$last_update = date("j M Y", strtotime($page_id->post_modified));
$get_encode = get_field( 'encode_site_links', 'options' );
$link_for_mobile = get_field('link_for_mobile', 'options');
$mba_m_text = get_field('mba_m_text', $page_id) ?? '';
$mba_m_link = get_field('mba_m_link', $page_id) ?? '';
?>
  <section class="review-page mobile-app <?= $dark_bg; ?>" <?= ($choose_bg) ? $review_bg_image : $review_bg_color;?>>
    <div class="container">
      ...
      <div class="review-page__wrap">
          ...
          <div class="review-page__rating">
            <?php if(!empty($review_rating)):?>
                  <div class="rating__star">
                    <span class="full" style="width: <?= $review_rating * 10?>%"></span> <span class="empty"></span>
                  </div>
            <?php endif;?>
        ...
        <div>
          ...
          <?php if($get_encode): ?>
              <div>
                <?= get_button($item['link'], $item['text'], 'review-page__link-bonus btn btn-with-back btn-big-text')?>
              </div>
          <?php else: ?>
            <div>
              <a class="review-page__link-bonus btn btn-with-back btn-big-text" href="<?= $item['link'];?>">
                <?php if($item['icon'] === 'android'): ?>
      ...
      <?php if(!$link_for_mobile && $mba_m_link !== ''): ?>
        <div class="mobile-version">
          <?= get_button($mba_m_link, $mba_m_text, 'btn btn-with-back', 'nofollow sponsored')?>
        </div>
      <?php endif;?>
    </div>
  </section>